...
template <typename T>
void multiset<T>::showTree_(Node* node, int mode) {
...
template <typename T>
void multiset<T>::showTree(int mode) {
    if(root)
        showTree_(root, mode);
    else 
...
template <typename T>
void multiset<T>::setColor(T key, bool color) {
...
template <typename T>
void multiset<T>::nodeInfo(Node* node) {
    ...
    //std::cout << "         high: " << node->high << "  |black high : " << node->highBlack << "  ";
